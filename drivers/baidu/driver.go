...
func (driver Baidu) Upload(file *model.FileStream, account *model.Account) error {
	...
	if count == 0 { //如果文件大小是0的话，直接使用文件md5当，block md5
		block_list = append(block_list, fmt.Sprintf("\"%s\"", content_md5))
	}
	_, err = tempFile.Seek(0, io.SeekStart)
	if err != nil {
		return err
	...
	aimPath := utils.Join(account.RootFolder, file.ParentPath, file.Name)
	path := encodeURIComponent(aimPath)
	...
	left = file.GetSize()
	totalLen := len(precreateResp.BlockList)
